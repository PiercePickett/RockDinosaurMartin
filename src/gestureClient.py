...
mp_draw = mp.solutions.drawing_utils # This draws the bones
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_lms, hand_info in zip(results.multi_hand_landmarks, results.multi_handedness):
            # 1. Draw the bones/landmarks on the image locally
            mp_draw.draw_landmarks(img, hand_lms, mp_hands.HAND_CONNECTIONS)
            
            # 2. Get Label (R/L)
            label = hand_info.classification[0].label 
            
            # 3. Simple Open/Fist Logic
            tip_ids = [8, 12, 16, 20]
